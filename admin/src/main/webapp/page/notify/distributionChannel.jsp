	...
	<div>
		<ul>
			<c:forEach var="channel" items="${channelList}">
				...
				<c:forEach var="myChannel" items="${myChannelList}">
					<c:if test="${myChannel.id == channel.id}">
						<c:set var="flag" value="true"></c:set>
					</c:if>
				</c:forEach>
				<c:if test="${flag == true}">
					<li class="checked" data-id="${channel.id}" >${channel.name}</li>
				</c:if>
				<c:if test="${flag == false}">
					<li data-id="${channel.id}">${channel.name}</li>
				</c:if>
			</c:forEach>
		</ul>
	</div>
	<form id="inputForm" action="${ctx}/notify/event/distribution.do" method="post">
