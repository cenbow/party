	...
	<section>
		<div class="section-main">
			<div class="c-time-list-header">
				<div class="r">
					<a href="${ctx}/goods/goods/dzGoodsForm.do" class="layui-btn layui-btn-radius layui-btn-danger"> <i class="iconfont icon-add btn-icon"></i>
						发布定制玩法
					</a>
				</div>
				<div class="ovh">
					<span class="title f20">定制玩法管理</span> <span class="f12">共<b id="totalCount">${page.totalCount}</b>条记录
					</span>
				</div>
			</div>
			<form class="layui-form" action="${ctx}/goods/goods/dzGoodsList.do" id="myForm" method="post">
			...
			<div class="c-time-list-content">
				<ul id="view">
					<c:if test="${page.totalCount == 0}">
						<div class="f16 tc mt15">还没有数据，请点击右上角的“发布定制玩法”</div>
					</c:if>
					<c:forEach var="goods" items="${goodsOutputs}">
						<li>
							<div class="detail-content">
								<a href="${ctx}/goods/goods/goodsDetail.do?id=${goods.id}&type=1" class="act-logo"
								...
								<div class="detail">
									<div class="act-title">
										...
										<a title="${goods.title}" href="${ctx}/goods/goods/goodsDetail.do?id=${goods.id}&type=1" class="title f18 ell db">${goods.title}</a>
									</div>
									<div>
										<p class="act-price">
									...
									<div class="opts-btns tb-opts"  style="width:475px">
										<div class="comm-opts">
											...
											<a href="${ctx}/goods/goods/dzGoodsForm.do?id=${goods.id}"> <i class="iconfont icon-edit btn-icon"></i> 编辑</a>
											...
											<c:if test="${goods.checkStatus == 1}">
												<a href="${ctx}/order/order/goodsOrderList.do?type=1&goodsId=${goods.id}"> <i class="iconfont icon-peoplelist btn-icon"></i> 预定管理</a>
											</c:if>
