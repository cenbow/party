	...
	<section>
		<div class="section-main">
			<div class="c-time-list-header">
				<div class="r">
					<a href="${ctx}/system/member/partnerForm.do" class="layui-btn layui-btn-radius layui-btn-danger">
						<i class="iconfont icon-add btn-icon"></i>添加合作商
					</a>
				</div>
				<div class="ovh">
					<span class="title f20">合作商管理</span>
					<span class="f12">共<b>${page.totalCount}</b>条记录</span>
				</div>
			</div>
			<form class="layui-form" action="${ctx}/system/member/partnerList.do" id="myForm" method="post">
						...
						<tbody>
							<c:forEach var="member" items="${members}">
								<tr>
									...
									<td  class="tb-opts">
										<div class="comm-opts">
											<a href="${ctx}/system/member/partnerForm.do?id=${member.id}" target="_self">
												编辑
											</a>
...
<!-- 											</a> -->
											...
											<a target="_self" href="javascript:openDialog('分配角色','${ctx}/system/role/distributionRole.do?memberId=${member.id}&type=1','400px','300px')">
												分配角色
											</a>
											<a target="_self" href="javascript:openDialog('分配资源','${ctx}/system/privilege/distribution/${member.id}.do','400px','300px')">
												分配资源
											</a>
...
//加载分页
	...
	function verify(content, status, targetId){
		layer.confirm(content, {icon: 3, title:'系统提示'}, function(index){
			window.location.href = "${ctx}/system/member/verify.do?id=" + targetId + "&userStatus=" + status;
			layer.close(index);
		});
	}
