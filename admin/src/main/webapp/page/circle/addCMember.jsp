	...
	<section>
		<div class="section-main">
			<div class="c-time-list-header">
				...
				<div class="ovh">
					<span class="title f20"> <a href="${ctx}/circle/list.do" class="">${circle.name}</a> > <a
						href="${ctx}/circle/member/list.do?id=${circle.id}">会员管理</a> > 添加会员
					</span>
				</div>
			</div>
			<!-- 正文请写在这里 -->
			<form id="infoForm" class="layui-form mt20" method="post" action="${ctx}/circle/member/save.do">
				<input type="hidden" name="cId" value="${circle.id}" /> <input type="hidden" name="ids"
					id="memberId" />
				<div class="layui-form-item">
					<div class="layui-inline">
						<label class="layui-form-label">真实姓名<span class="f-verify-red">*</span></label>
						<div class="layui-input-inline">
							<input class="layui-input" name="realname" lay-verify="realname" placeholder="真实姓名"
								id="realName" />
						</div>
					</div>
				</div>
				...
				<div class="layui-form-item">
					<div class="layui-inline">
						...
						<div class="layui-input-inline">
							<input name="mobile" lay-verify="mobile" class="layui-input" id="phone">
						</div>
					</div>
				</div>
				<div class="layui-form-item">
				...
				<div class="layui-form-item">
					<div class="layui-input-block">
						<a href="javascript:void(0)" class="layui-btn layui-btn-danger" lay-submit
							lay-filter="infoForm" id="saveMember">立即提交</a> <a href="javascript:addMember()"
							class="layui-btn layui-btn-danger" id="addMember" style="display: none;">立即提交</a> <a
							href="${ctx}/circle/member/list.do?id=${circle.id}" class="layui-btn layui-btn-primary">取消</a>
					</div>
				</div>
			</form>
		</div>
...
<script type="text/javascript" charset="utf-8"
	src="${ctxStatic}/bootstrap3-typeahead/bootstrap3-typeahead.js"></script>
<script>
	var flag = false;
                ...
                $.post(action, $('#infoForm').serialize(), function(res) {
                    if (res.success) {
                        layer.msg('提交成功', {
                            ...
                            location.href = "${ctx}/circle/member/list.do?id=${circle.id}";
                        });
                    } else {
                        layer.msg('提交失败', {
                            icon : 2
...
// 		}
	...
	// 添加成员
	function addMember() {
		$.post('${ctx}/circle/member/save.do', {
			cId : '${circle.id}',
			ids : $("#memberId").val()
		}, function(res) {
			if (res.success) {
