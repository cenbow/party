    ...
    <section>
        <div class="section-main">
            <div class="c-time-list-header">
                <div class="r">
                    <a href="${ctx}/circle/tag/form.do?circleId=${circle.id}" class="layui-btn layui-btn-radius layui-btn-danger">
                        <i class="iconfont icon-add btn-icon"></i> 创建用户类型
                    </a>
                </div>
                <div class="ovh">
                    <span class="title f18">用户类型管理</span>
                    <span class="f12">共<b>${page.totalCount}</b>条记录</span>
                </div>
            </div>
            <form class="layui-form" action="${ctx}/circle/tag/list.do" id="myForm" method="post">
                        ...
                        <ul class="search-form-content">
                            <li class="form-item-inline"><label class="search-form-lable">名称</label>
                                <div class="layui-input-inline">
                                    <input type="text" name="tagName" autocomplete="off" class="layui-input" value="${tag.tagName}" placeholder="请输入查询类型名称">
                                </div>
                            </li>
                    ...
                    <table class="layui-table">
                        ...
                        <c:forEach var="tag" items="${list}">
                            <tr>
                                ...
                                <td class="tb-opts">
                                    <div class="comm-opts">
                                        <a target="_self" class="green" href="${ctx}/circle/tag/form.do?circleId=${circle.id}&id=${tag.id}">
                                            编辑
                                        </a>
...
<script type="text/javascript">
    ...
    function delType(id) {
        layer.confirm('确定删除该类型么？', {icon: 3, title: '提示'}, function (index) {
            $.post('${ctx}/circle/tag/del.do', {id: id}, function (res) {
                if (res.success) {
