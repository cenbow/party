	...
	<section>
		<div class="section-main">
			<div class="c-time-list-header">
				<div class="ovh">
					<span class="title f20">动态管理</span> <span class="f12">共<b id="totalCount">${page.totalCount}</b>条记录
					</span>
				</div>
			</div>
			<form class="layui-form" action="${ctx}/dynamic/dynamic/dynamicList.do" id="myForm" method="post">
						...
						<ul class="search-form-content">
							<li class="form-item-inline"><label class="search-form-lable">内容</label>
								<div class="layui-input-inline">
									<input type="text" name="content" autocomplete="off" class="layui-input" value="${dynamic.content}" placeholder="请输入查询动态内容">
								</div>
							</li>
							<li class="form-item-inline"><label class="search-form-lable">发布者</label>
								<div class="layui-input-inline">
									<input type="text" name="memberName" autocomplete="off" class="layui-input" value="${input.memberName}" placeholder="请输入查询动态发布者">
								</div>
							</li>
								...
								<div class="detail">
									<div class="act-title">
										<div class="user-infos l">
											...
											<div class="l pt10 ml5"><span class="f16">${dynamic.authorMember.realname}</span></div>
										...
										<span class="status f16" style="height: 44px;line-height: 44px;"><fmt:formatDate value="${dynamic.updateDate}" pattern="yyyy-MM-dd HH:mm"/></span>
										<div class="cl"></div>
									</div>
									<div class="act-title">
										${dynamic.content}
									...
									<div class="act-title">
										<c:forEach var="pic" items="${dynamic.picList}">
											<div class="bg-avater-big" style="background-image: url('${pic.picUrl}'),url(${ctx}/image/img_bg.png)"></div>
										</c:forEach>
									</div>
									...
									<div class="opts-btns tb-opts" style="width:475px">
										<div class="comm-opts">
											...
											<a href="${ctx}/dynamic/dynamic/dynamicForm.do?id=${dynamic.id}"> <i class="iconfont icon-edit btn-icon"></i> 编辑</a><span>|</span>
											<a href="javascript:delDynamic('${dynamic.id}');">
												<i class="iconfont icon-delete btn-icon"></i> 删除
											</a><span>|</span>
											<a href="${ctx}/dynamic/love/loveList.do?refId=${dynamic.id }"><i class="iconfont icon-like btn-icon"></i>点赞管理</a><span>|</span>
											<a href="${ctx}/dynamic/comment/commentList.do?refId=${dynamic.id}"> <i class="iconfont icon-peoplelist btn-icon"></i>
												评论管理
											</a>
										</div>
									</div>
        ...
        layer.confirm('确定要删除该动态么？', {
                ...
                function del() {
                    ...
                    if (fileId) {
                        ...
                        var url = '${ctx}/dynamic/dynamic/delete.do?id='+id;
