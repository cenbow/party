        ...
        <div class="section-main">
            ...
            <div class="layui-tab layui-tab-brief" lay-filter="zc">
                <ul class="layui-tab-title" id="zc_tab">
                    <li <c:if test="${empty groupMember.gradeStatus}">class="layui-this"</c:if>>全部</li>
                    <li <c:if test="${groupMember.gradeStatus == 0}">class="layui-this"</c:if>>无效</li>
                    <li <c:if test="${groupMember.gradeStatus == 1}">class="layui-this"</c:if>>有效</li>
                </ul>
                <div class="layui-tab-content">
                    <form class="layui-form" action="${ctx}/sign/member/list.do" id="myForm" method="post">
                        <input type="hidden" name="pageNo" id="pageNo" />
                        <input type="hidden" name="gradeStatus" id="gradeStatus" value="${groupMember.gradeStatus}"/>
                            ...
                            <table class="layui-table">
                                ...
                                <c:forEach var="apply" items="${list}">
                                    <tr>
                                        ...
                                        <td>
                                            <c:if test="${apply.gradeStatus == 0}">
                                                无效
                                            </c:if>
                                            <c:if test="${apply.gradeStatus == 1}">
                                                有效
                                            </c:if>
                                        </td>
                                        <td  class="tb-opts">
                                    ...
                                    <%--            <c:if test="${apply.status == 0}">
                                                ...
                                                <c:if test="${apply.gradeStatus == 0}">
                                                    <a href="javascript:checkGrade('${apply.id}', 1)" target="_self">
                                                        成绩有效
                                                    </a>
                                                </c:if>
                                                <c:if test="${apply.gradeStatus == 1}">
                                                    <a href="javascript:checkGrade('${apply.id}', 0)" target="_self">
                                                        成绩无效
                                                    </a>
                                                </c:if>
...
<script type="text/javascript">
    ...
    function checkGrade(id, status) {
        ...
        layer.confirm(info, {
            ...
            $.post("${ctx}/sign/member/checkGrade.do", {id : id,status: status}, function(data) {
                if (data.success == true) {
                    layer.alert("审核成功", {
                        icon : 6
                    }, function(index) {
