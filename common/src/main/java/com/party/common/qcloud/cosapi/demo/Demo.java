 ...
 */
package com.party.common.qcloud.cosapi.demo;

import com.party.common.qcloud.cosapi.api.*;
...
public class Demo {
    ...
    private static final int APP_ID = 10052192;
    private static final String SECRET_ID = "AKID0zLb467fRifqb0rsyo5tVJKZqKiebe5B";
    private static final String SECRET_KEY = "UcfDJpkyGi90IDw0YsnxxUcLkWopbrC6";

    // 通过控制台提前建好的bucket
    private static final String bucketName = "tongxingzhe";

    // 默认获取的目录列表的数量
    private static final int DIR_NUM = 20;
    // 用来测试上传文件夹的目录
    private static final String DIR_REMOTE_PATH = "/userfiles/activity/";

    // 用来测试单文件上传的小文件的远程路径与本地路径
    private static final String SMALLFile_REMOTE_PATH = DIR_REMOTE_PATH + "/len10.txt";
    private static final String SMALLFile_LOCAL_PATH = "src/test/resources/len10.txt";

    // 用来测试分片串行上传的远程路径与本地路径
    private static final String SLICEFILE_REMOTE_PATH = DIR_REMOTE_PATH + "/slicefile.txt";
    private static final String SLICEFILE_LOCAL_PATH = "src/test/resources/slicefile.txt";

    // 用来测试分片并行上传的远程路径与本地路径
    private static final String PARA_SLICEFILE_REMOTE_PATH = DIR_REMOTE_PATH+ "/para_slicefile.txt";
